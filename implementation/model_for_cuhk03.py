...
class NumpyArrayIterator_for_CUHK03(pre_image.Iterator):
    ...
    def next(self):
        ...
        print("this is [batch_x1,batch_x2], batch_y")
...
class ImageDataGenerator_for_multiinput(pre_image.ImageDataGenerator):
    ...
    def flow(self, f, train_or_validation = 'train', flag = 0, batch_size=150, shuffle=True, seed=1217):
        print("this is NumpyArrayIterator")
        print(NumpyArrayIterator_for_CUHK03(f, train_or_validation, flag, self, batch_size=batch_size, shuffle=shuffle, seed=seed))
...
def test(model,val_or_test='test'):
    ...
    print("this is model.predict_on_batch([a,b])")
    print(model.predict_on_batch([a,b]))
...
def cmc(model, val_or_test='test'):
        ...
        print("this is _cmc_curve(model,a,b)")
        print(_cmc_curve(model,a,b))
...
#train_num=100,one_epoch=30000
def train(model,weights_name='weights_on_cuhk03_0_0',train_num=10,one_epoch=30,epoch_num=1,flag_random=None,random_pattern=lambda x:x/2+0.4,flag_train=0,flag_val=1,which_val_data='validation',nb_val_samples=1000):
    with h5py.File('cuhk-03.h5','r') as f:
        ...
        print("this is Rank1s")
        print(Rank1s)
        return Rank1s


if __name__ == '__main__':
    print ('default dim order is hehe:',K.image_dim_ordering())
    user_name = input('please input your system user name:')
    model = model_def()
    print ('model definition done desu.')
